...
//            mRestaurant = (Restaurant) getArguments().getSerializable("restaurant");
        ...
        if (mRestaurantName.equals("동국반점")) {
            for (int i = 0; i < list.size(); i++) {
                mMenuList.add(new Menu(list.get(i).getName(),list.get(i).getPriceAmount(),list.get(i).getDescription(),R.drawable.china_1));
            }

        } else if (mRestaurantName.equals("동국반점 2호점")) {
            for (int i = 0; i < list.size(); i++) {
                mMenuList.add(new Menu(list.get(i).getName(),list.get(i).getPriceAmount(),list.get(i).getDescription(),R.drawable.china_2));
            }
        }
